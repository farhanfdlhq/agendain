...
export default function SettingsPage() {
  ...
  const [formData, setFormData] = useState({
    site_name: "Agendain",
    whatsapp_number: "6281995264565",
    whatsapp_message:
      "Halo Agendain, saya ingin bertanya tentang paket wisata.",
    payment_instructions:
      "Silakan transfer ke rekening BCA 1234567890 a.n PT Agendain.",
    site_logo: "/logo.png",
    site_favicon: "/favicon.ico",
    logo_height: "42",
    global_informasi_penting:
  ...
  const handleWhatsAppBlur = () => {
    if (formData.whatsapp_number) {
      setFormData((prev) => ({
        ...prev,
        whatsapp_number: formatWhatsAppNumber(prev.whatsapp_number),
      }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Endpoint generik POST /api/settings MENOLAK (403) key yang punya route &
    // validasi sendiri: apa pun yang berakhiran "_settings" (home/theme/about/
    // footer/invoice/dst) dan "roles_config", plus key pencemar prototype. GET
    // mengembalikan key-key itu ke Super Admin sehingga ikut termuat ke formData;
    // bila dikirim balik, SELURUH simpan gagal 403 (bukan cuma logo/favicon).
    // Halaman ini hanya mengurus identitas & parameter sistem, jadi key tersebut
    // memang bukan urusannya — dibuang sebelum kirim.
    const DENIED_KEYS = new Set(["roles_config", "__proto__", "constructor", "prototype"]);
    const payload = Object.fromEntries(
      Object.entries(formData).filter(
        ([k]) => !DENIED_KEYS.has(k) && !k.endsWith("_settings"),
      ),
    );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Konfigurasi parameter dan identitas utama website Agendain.
          </p>
        </div>
        <Button
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2">
                  <MediaPicker
                    ...
                    onChange={(url) =>
                      setFormData((prev) => ({ ...prev, site_logo: url }))
                    }
                    label="Pilih Logo"
              ...
              <Alert variant="info" className="shadow-xs">
                ...
                <AlertDescription className="text-xs leading-relaxed">
                  <p>
                    ...
                    (mendukung transparansi) atau{" "}
                    <span className="font-semibold text-foreground">ICO</span>,
                    sebaiknya berwujud{" "}
                    <span className="font-semibold text-foreground">
                      Bujur Sangkar (contoh: 512×512 piksel)
                    </span>
                    , maksimal{" "}
                    <span className="font-semibold text-foreground">10MB</span>.
                    File PNG disimpan tetap PNG (tidak dikonversi ke WebP), dan
                    file ICO diunggah apa adanya agar langsung termuat di tab
                    semua perangkat.
                  </p>
                </AlertDescription>
              </Alert>
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="w-12 text-sm font-medium">
                  {formData.logo_height || "42"}px
                </span>
              </div>
              <p className="text-[11px] text-muted-foreground">
                Sesuaikan ukuran tampilan logo pada Navbar dan Footer (Default:
                42px).
              </p>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                Otomatis diformat ke awalan 62 saat Anda klik di luar kolom.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                Pesan ini otomatis terisi saat kustomer klik tombol chat WA.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                Instruksi ini akan ditampilkan kepada kustomer setelah booking.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Pengaturan ini akan digunakan pada paket yang tidak memiliki
              kebijakan custom secara spesifik.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="global_informasi_penting">
                Informasi Penting (Default)
              </Label>
              <Textarea
              ...
              />
              <Label
                htmlFor="global_informasi_penting_en"
                className="text-xs text-muted-foreground font-normal pt-1"
              >
                🇬🇧 English (opsional — kosongkan untuk pakai versi Indonesia)
              </Label>
              <Textarea
                ...
                rows={4}
                placeholder={
                  formData.global_informasi_penting ||
            ...
            <div className="space-y-2">
              <Label htmlFor="global_kebijakan_pembatalan">
                Kebijakan Pembatalan & Pengembalian Dana (Default)
              </Label>
              <Textarea
              ...
              />
              <Label
                htmlFor="global_kebijakan_pembatalan_en"
                className="text-xs text-muted-foreground font-normal pt-1"
              >
                🇬🇧 English (opsional — kosongkan untuk pakai versi Indonesia)
              </Label>
              <Textarea
                ...
                rows={4}
                placeholder={
                  formData.global_kebijakan_pembatalan ||
                  "Write each point on a new line..."
                }
              />
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="global_opsi_penjemputan">
                Opsi Penjemputan (Default)
              </Label>
              <Textarea
              ...
              />
              <Label
                htmlFor="global_opsi_penjemputan_en"
                className="text-xs text-muted-foreground font-normal pt-1"
              >
                🇬🇧 English (opsional — kosongkan untuk pakai versi Indonesia)
              </Label>
              <Textarea
                ...
                rows={4}
                placeholder={
                  formData.global_opsi_penjemputan ||
                  "Write each point on a new line..."
                }
              />
            </div>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Geser dan perbesar gambar untuk menyesuaikan posisi Favicon Anda
              (berbentuk kotak).
            </DialogDescription>
          </DialogHeader>
