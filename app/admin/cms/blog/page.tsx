...
export default function BlogCMSPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Header halaman Blog berhasil diperbarui!')
      } else {
  ...
  const renderImageInput = (label: string, fieldName: string, placeholder = '', resolutionHint = '') => (
    <div className="space-y-3">
      ...
      <MediaPicker
        ...
        onChange={(url) => setData((prev: any) => ({ ...prev, [fieldName]: url }))}
        label="Pilih Gambar"
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto py-4">
      <div ref={topHeaderRef} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">CMS Halaman Blog</h2>
          <p className="text-muted-foreground text-sm mt-1">Kelola header (hero) di halaman daftar artikel /blog.</p>
        </div>
        <Button
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="border shadow-sm overflow-hidden">
          <CardHeader className="bg-slate-50/50 border-b pb-4">
            <CardTitle className="text-lg text-primary">Bagian Hero (Header Blog)</CardTitle>
            <CardDescription>Judul, sub-judul, dan gambar latar di bagian atas halaman /blog.</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="space-y-4">
                {renderImageInput('Gambar Latar Belakang (Hero Image)', 'heroImage', '/dest-italy.webp', '1920x1080 px (Landscape)')}
              </div>
              <div className="space-y-4 flex flex-col justify-start">
                {renderTextInput('Judul Utama (*Highlight Kuning*)', 'heroTitle', false, 'Jurnal *Agendain*', true, '800')}
                {renderTextInput('Sub-Judul (Deskripsi)', 'heroSubtitle', true, 'Temukan inspirasi liburan impianmu...', true, '500')}
              </div>
            </div>
          </CardContent>
        </Card>
