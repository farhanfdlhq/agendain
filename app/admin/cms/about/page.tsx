...
export default function AboutCMSPage() {
  const [data, setData] = useState<any>({
    ...
    heroImage: '',
    missionLabel: '', missionLabel_en: '', missionLabelWeight: '600',
    missionTitle: '', missionTitle_en: '', missionTitleWeight: '800',
    missionDesc: '', missionDesc_en: '', missionDescWeight: '500',
    missionImage: '',
    guidePill: '', guidePill_en: '',
    guideTitle: '', guideTitle_en: '', guideTitleWeight: '800',
    guideDescP1: '', guideDescP1_en: '',
    guideDescP2: '', guideDescP2_en: '', guideDescWeight: '500',
    guideImage: '',
  })
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto py-4">
      <div ref={topHeaderRef} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">CMS Tentang Kami</h2>
          <p className="text-muted-foreground text-sm mt-1">Kelola konten statis di halaman Tentang Kami.</p>
        </div>
        <Button 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card className="border shadow-sm overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b pb-4">
              <CardTitle className="text-lg text-primary">Bagian Hero (Beranda Utama)</CardTitle>
              <CardDescription>Header dengan gambar latar belakang.</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-4 flex flex-col justify-start">
                  {renderTextInput('Judul Utama (*Highlight Kuning*)', 'heroTitle', false, 'Apa itu *Agendain*?', true, '800')}
                  {renderTextInput('Sub-Judul (Deskripsi)', 'heroSubtitle', true, 'Bagi kami, menjelajahi Eropa...', true, '500')}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border shadow-sm overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b pb-4">
              <CardTitle className="text-lg text-primary">Bagian Misi Kami</CardTitle>
              <CardDescription>Kartu misi yang menumpuk di atas bagian Hero.</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-4">
                  {renderImageInput('Gambar Misi', 'missionImage', '/dest-italy.webp', '1080x1080 px (Square/Landscape)')}
                </div>
                <div className="space-y-4 flex flex-col justify-start">
                  {renderTextInput('Label Kecil Atas', 'missionLabel', false, 'Misi kami', true, '600')}
                  {renderTextInput('Judul Misi', 'missionTitle', false, 'Mengubah rencana liburanmu...', true, '800')}
                  {renderTextInput('Deskripsi Misi', 'missionDesc', true, 'Di Agendain, fokus kami adalah...', true, '500')}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border shadow-sm overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b pb-4">
              <CardTitle className="text-lg text-primary">Bagian Lead Guide</CardTitle>
              <CardDescription>Profil pemandu wisata utama.</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-4">
                  {renderImageInput('Foto Lead Guide', 'guideImage', '/why-camera.webp', '800x1200 px (Portrait)')}
                </div>
                <div className="space-y-4 flex flex-col justify-start">
                  {renderTextInput('Label Kecil Atas', 'guidePill', false, 'Dibalik Agendain')}
                  {renderTextInput('Judul Utama (*Highlight Kuning*)', 'guideTitle', false, 'Meet Our *Lead Guide*', true, '800')}
                  {renderTextInput('Deskripsi Paragraf 1 (Kutipan)', 'guideDescP1', true, '"Eropa punya sejuta pesona..."')}
                  {renderTextInput('Deskripsi Paragraf 2', 'guideDescP2', true, 'Kenalin, sosok di balik setiap rute...', true, '500')}
                </div>
              </div>
            </CardContent>
          </Card>
