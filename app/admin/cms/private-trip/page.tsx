...
function BadgeInput({ tags, onChange, placeholder }: { tags: string[], onChange: (tags: string[]) => void, placeholder?: string }) {
    ...
    <div className="flex flex-col gap-2 bg-white rounded-xl border p-3">
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, idx) => (
            <div key={idx} className="flex items-center gap-1.5 bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full text-xs font-medium border border-blue-100">
              {tag}
              <button 
                type="button" 
      ...
      <div className="relative mt-1">
        ...
        />
        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded border">+ Tambah</span>
      </div>
      <p className="text-[10px] text-muted-foreground mt-1">Tekan Enter untuk menambahkan badge/chips.</p>
    </div>
  )
}
...
export default function PrivateTripCMSPage() {
  const [data, setData] = useState<any>({
    ...
    ctaTitle: '', ctaTitle_en: '', ctaTitleWeight: '800',
    ctaSubtitle: '', ctaSubtitle_en: '', ctaSubtitleWeight: '500',
    ctaBtnText: '', ctaBtnText_en: '',
  })
  ...
  const renderPackageEditor = () => {
    ...
    const handleAddItem = () => {
      const newItem = { title: '', subtitle: '', image: '', locationTab: '', chips: [], features: [] };
    ...
    const handleArrayStringChange = (index: number, field: string, text: string) => {
      const newItems = [...items];
      newItems[index] = { ...newItems[index], [field]: text.split('\n').map(s => s.trim()).filter(s => s !== '') };
      setData((prev: any) => ({ ...prev, [activeFieldName]: newItems }));
    };
    ...
    return (
      <Card className="border shadow-sm overflow-hidden">
        <CardHeader className="bg-slate-50/50 border-b pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <CardTitle className="text-lg text-primary">Manajemen Kartu Paket Private Trip</CardTitle>
        ...
        <CardContent className="pt-6">
          ...
          <div className="space-y-6">
            {items.map((item: any, index: number) => (
              <div key={index} className="p-4 border rounded-xl bg-muted/10 relative shadow-sm">
                <Button 
                  type="button" 
                  variant="destructive" 
                  size="icon" 
                ...
                <h4 className="text-sm font-semibold mb-4">Paket #{index + 1}</h4>
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label className="text-xs">Label Lokasi (Kiri Atas)</Label>
                      <Input 
                        value={item.locationTab || ''} 
                        onChange={(e) => handleItemChange(index, 'locationTab', e.target.value)}
                    ...
                    <div className="space-y-2">
                      ...
                      <BadgeInput 
                        ...
                        onChange={(newTags) => handleItemChange(index, 'chips', newTags)}
                  ...
                  <div className="space-y-4 flex flex-col justify-start">
                    <div className="space-y-2">
                      <Label className="text-xs">Nama Paket (Kecil Abu-abu)</Label>
                      <Input 
                        value={item.subtitle || ''} 
                    ...
                    <div className="space-y-2">
                      <Label className="text-xs">Poin Fitur/Keunggulan (Pisahkan dengan Enter)</Label>
                      <Textarea 
                        value={(Array.isArray(item.features) ? item.features : []).join('\n')} 
                        onChange={(e) => handleArrayStringChange(index, 'features', e.target.value)}
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto py-4">
      <div ref={topHeaderRef} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">CMS Private Trip</h2>
          <p className="text-muted-foreground text-sm mt-1">Kelola teks untuk halaman Private Trip Eropa.</p>
        </div>
        <Button 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card className="border shadow-sm overflow-hidden">
            ...
            <CardContent className="pt-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-4">
                  {renderImageInput('Gambar Latar Belakang (Hero Image)', 'heroImage', '/private_trip_hero.png', '1920x1080 px (Landscape)')}
                </div>
                <div className="space-y-4 flex flex-col justify-start">
                  {renderTextInput('Judul Utama (*Highlight Kuning*)', 'heroTitle', false, 'Eropa Eksklusif *Sesuai Cara* Kamu', true, '800')}
                  {renderTextInput('Sub-Judul (Deskripsi)', 'heroSubtitle', true, 'Rancang perjalanan impianmu...', true, '500')}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border shadow-sm overflow-hidden">
            ...
            <CardContent className="pt-6 grid gap-6">
              {renderTextInput('Judul Daftar Paket (*Highlight Kuning*)', 'packagesTitle', false, 'Pilihan *Private Trip* Kami', true, '800')}
              {renderTextInput('Deskripsi Daftar Paket', 'packagesSubtitle', true, 'Beragam kelas layanan yang disesuaikan...', true, '500')}
            </CardContent>
          </Card>
          ...
          <Card className="border shadow-sm overflow-hidden">
            ...
            <CardContent className="pt-6 grid gap-6">
              {renderTextInput('Judul Promo (*Highlight Kuning*)', 'ctaTitle', false, 'Siap *Agendain* Private Trip Kamu?', true, '800')}
              {renderTextInput('Sub-Judul (Deskripsi)', 'ctaSubtitle', true, 'Mari diskusikan rencana perjalanan impianmu...', true, '500')}
              {renderTextInput('Teks Tombol', 'ctaBtnText', false, 'Hubungi Konsultan Kami')}
            </CardContent>
          </Card>
