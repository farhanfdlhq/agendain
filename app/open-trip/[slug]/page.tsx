...
export default async function PaketDetail(props: {
  ...
  const [pkg, settingsArr, rates, locale, t, privatePackages] =
    await Promise.all([
  ...
  let settingsObj = (settingsArr as any[]).reduce(
  ...
  const paidAgg = await prisma.booking
    .aggregate({
      where: { openTripId: pkg.id, status: "paid" },
      _sum: { jumlahPax: true },
    })
  ...
  if (Array.isArray(infoPenting) && infoPenting.length > 0) {
    ...
    finalInfoPenting = gset("global_informasi_penting")!
      .split("\n")
  ...
  if (Array.isArray(kebijakan) && kebijakan.length > 0) {
    ...
    finalKebijakan = gset("global_kebijakan_pembatalan")!
      .split("\n")
      .filter((s: string) => s.trim());
  } else {
    finalKebijakan = [
  ...
  const akomodasi = (
    pickLocalized<any[]>(pkg, "akomodasi", locale) || []
  ).filter(Boolean);
  const penerbangan = (
    pickLocalized<any[]>(pkg, "penerbangan", locale) || []
  ).filter(Boolean);
  ...
  if (Array.isArray(pkg?.foto) && pkg.foto.length > 0) {
    gallery = pkg.foto.map((f: any) => {
      ...
      if (typeof f === "object" && f !== null)
        return f.full || f.medium || f.thumb || "/placeholder.webp";
    ...
    mainImage =
      fotos.full || fotos.large || fotos.medium || "/placeholder.webp";
  ...
  const tidakTermasuk =
    pickLocalized<string[]>(pkg, "tidakTermasuk", locale) || [];
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <div className={styles.breadcrumb}>
          ...
          <Link href="/open-trip">Open Trip</Link>{" "}
          <span className={styles.separator}>/</span>
        ...
        <div className={styles.quickPanel}>
          <div
            className={`${styles.departureRow} ${tripBerakhir ? styles.departureEnded : ""}`}
          >
            <CalendarClock className={styles.departureIcon} />
            <div className={styles.quickText}>
              <span className={styles.departureLabel}>
                {t("openTrip.detail.departure")}
              </span>
            ...
            <div className={styles.quickInfoCard}>
              ...
              <div className={styles.quickText}>
                <span className={styles.quickLabel}>
                  {t("openTrip.detail.duration")}
                </span>
                <span className={styles.quickValue}>
                  {pkg.durasi} {t("openTrip.detail.days")}
                </span>
              </div>
            </div>
            <div className={styles.quickInfoCard}>
          ...
          {kuota !== null && (
            <div className={styles.participantRow}>
              ...
              <div className={styles.quickText}>
                <span className={styles.quickLabel}>
                  {t("openTrip.detail.participants")}
                </span>
                {/* Setelah trip berakhir, sisa kursi tidak relevan lagi. */}
                <span
                  className={`${styles.quickValue} ${tripBerakhir ? styles.seatsFull : sisaKursi === 0 ? styles.seatsFull : styles.seatsLeft}`}
                >
                  {tripBerakhir
                    ? t("openTrip.detail.tripEnded")
                    : sisaKursi === 0
            ...
            <section className={styles.section}>
              ...
              <div className={styles.incExcGrid}>
                ...
                <div className={styles.excBox}>
                  <h3 className={styles.subTitle}>
                    {t("openTrip.detail.excluded")}
                  </h3>
                  <ul className={styles.listExc}>
                    {tidakTermasuk.map((item, i) => (
                      <li key={i}>{item}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className={styles.itineraryContainer}>
                  {itinerary.map((it, i) => {
                    ...
                    const regex = new RegExp(
                      `^(Hari|Day|${dayWord})\\s*${it.hari}\\s*[-:–]*\\s*`,
                      "i",
                    );
                    let cleanJudul = rawJudul.replace(regex, "").trim();
                    // Jika judul aslinya cuma "Hari 1", cleanJudul jadi kosong
                    if (
                      cleanJudul.toLowerCase() === `hari ${it.hari}` ||
                      cleanJudul.toLowerCase() ===
                        `${dayWord} ${it.hari}`.toLowerCase()
                    ) {
                      cleanJudul = "";
                    }
                      ...
                      {akomodasi.map((item: any, idx: number) => (
                        <li key={idx}>
                          {typeof item === "string" ? (
                            ...
                            <>
                              {item.kota && (
                                <span className={styles.logisticsLabel}>
                                  {item.kota}
                                </span>
                              )}
                        ...
                        <li key={idx}>
                          {typeof item === "string" ? (
                            ...
                            <>
                              ...
                              {item.detail && (
                                <span className={styles.flightDetail}>
                                  {item.detail}
                                </span>
                              )}
                              {item.maskapai && (
                                <span className={styles.flightAirline}>
                                  {item.maskapai}
                                </span>
                              )}
                            </>
          ...
          <div className={styles.sidebar}>
            <div className={styles.resCard}>
                ...
                <div className={styles.rateNote}>
                  ...
                  {rates.source === "wise" ? (
                    <>
                      {" "}
                      ·{" "}
                      <span className={styles.rateLive}>
                        {t("openTrip.detail.rateLive")}
                      </span>
                    </>
                  ) : (
                    <>
                      {" "}
                      ·{" "}
                      <span className={styles.rateStale}>
                        {t("openTrip.detail.rateFallback")}
                      </span>
                    </>
                  )}
                </div>
              ...
              <BookingForm
                ...
                whatsappNumber={settingsObj.whatsapp_number || "6281995264565"}
                fixedDeparture={
                  pkg.tanggalKeberangkatan
                    ? pkg.tanggalKeberangkatan.toISOString().slice(0, 10)
                    : null
                }
                fixedDepartureLabel={
                  pkg.tanggalKeberangkatan ? tanggalBerangkat : undefined
                }
                sisaKursi={sisaKursi}
              ...
              <div className={styles.waOption}>
                ...
                <a
                  href={`https://wa.me/${settingsObj.whatsapp_number?.replace(/\D/g, "") || "6281995264565"}?text=${encodeURIComponent(`${t("openTrip.detail.waText")} ${nama}`)}`}
                  target="_blank"
                  rel="noreferrer"
        ...
        {privatePackages.length > 0 && (
          <section className={styles.recommendSection}>
            <h2 className={styles.sectionTitle}>
              {t("openTrip.detail.privateRecommendation")}
            </h2>
            <p className={styles.recommendDesc}>
              {t("openTrip.detail.privateRecommendationDesc")}
            </p>
              ...
              {privatePackages.map((tier: any) => (
                ...
                >
                  ...
                  <div className={styles.recommendBody}>
                    <span className={styles.recommendSubtitle}>
                      {tier.subtitle}
                    </span>
                    <h3 className={styles.recommendTitle}>{tier.title}</h3>
                    <span className={styles.recommendLink}>
                      {t("openTrip.detail.seeDetail")} →
                    </span>
                  </div>
                </Link>
              ))}
